...
const FONNTE_TOKEN       = PropertiesService.getScriptProperties().getProperty('FONNTE_TOKEN') || "NPUQeEn4zATP628wK7au";
...
function handleKirimHasilSurvey(requestData) {
    ...
    const waResult = kirimWA(data.hp, data.nama, fileUrl, data.sponsor, data.hpSponsor);
    const emailResult = data.email
      ? kirimEmail(data.email, data.nama, fileUrl, data.sponsor, data.hpSponsor)
      : { success: false, skipped: true, message: 'Email tidak diisi.' };
    ...
    const isAnySuccess = Boolean(waResult.success || emailResult.success);
    const status = isAnySuccess ? "success" : "error";
    const message = isAnySuccess
      ? "Proses kirim hasil survey selesai."
      : `Gagal mengirim hasil survey. ${waResult.message || emailResult.message || ''}`.trim();
...
function kirimEmail(email, nama, fileUrl, mSponsor, mHpSponsor) {
  try {
    ...
    if (email && email.includes("@")) {
      ...
      return { success: true, target: email, message: "Email berhasil dikirim." };
    }

    return { success: false, skipped: true, target: email || '', message: "Alamat email tidak valid atau kosong." };
  } catch (e) {
    console.error("Gagal kirim email:", e);
    return { success: false, target: email || '', message: e.message || "Gagal mengirim email." };
  }
}
...
function kirimWA(nomorHP, nama, fileUrl, mSponsor, mHpSponsor) {
  ...
  const noWaUser    = normalizeWhatsAppNumber(nomorHP);
  const noWaSponsor = normalizeWhatsAppNumber(mHpSponsor);
  ...
  const userResult = sendWhatsAppFonnte(noWaUser, fPesanWA);
  if (userResult.success) {
    console.log("Pesan WA terkirim ke konsumen:", noWaUser);
  } else {
    console.error("Gagal kirim WA ke konsumen:", userResult.message);
  }

  const sponsorResult = sendWhatsAppFonnte(noWaSponsor, "*Notifikasi List Baru*\n" + fPesanWA);
  if (sponsorResult.success) {
    console.log("Pesan WA terkirim ke sponsor:", noWaSponsor);
  } else {
    console.error("Gagal kirim WA ke sponsor:", sponsorResult.message);
  }

  const success = Boolean(userResult.success || sponsorResult.success);
  const message = success
    ? "Pesan WA berhasil diproses."
    : `WA user: ${userResult.message || 'gagal'} | WA sponsor: ${sponsorResult.message || 'gagal'}`;
